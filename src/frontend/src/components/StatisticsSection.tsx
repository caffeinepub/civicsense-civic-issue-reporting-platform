...
export default function StatisticsSection() {
  ...
  const totalSolved = allIssues.filter(
    (issue) =>
      issue.status === Status.resolved || issue.status === Status.closed,
  ).length;
  const solveRate =
    totalRaised > 0 ? Math.round((totalSolved / totalRaised) * 100) : 0;
      ...
      <div className="container px-4">
        <div className="mb-8 text-center">
          ...
          <p className="text-muted-foreground">
            Track our progress in making our community better
          </p>
        </div>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <Card className="group transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
            ...
            <CardContent>
              <div className="text-4xl font-bold transition-all duration-300 group-hover:scale-110">
                {totalRaised}
              </div>
          ...
          <Card className="group transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
            ...
            <CardContent>
              <div className="text-4xl font-bold text-success transition-all duration-300 group-hover:scale-110">
                {totalSolved}
              </div>
              <p className="mt-2 text-xs text-muted-foreground">
